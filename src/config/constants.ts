import {Building2, CalendarClock, Gauge, Star, TrendingUp} from "lucide-react";
...
export const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL?.trim() ?? 'http://localhost:8000';
export const WEBSOCKET_URL = API_BASE_URL.replace(/^http/i, 'ws') + '/ws/compare';
export const GOOGLE_MAPS_API_KEY_FROM_ENV = process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || "";

export const SORT_OPTIONS: SortOption[] = [{key: 'recommended', label: 'Recommended', icon: Star}, {
    key: 'price_asc', label: 'Price (Lowest First)', icon: TrendingUp
}, {key: 'speed_desc', label: 'Speed (Fastest Download)', icon: Gauge}, {
    key: 'duration_asc', label: 'Contract (Shortest First)', icon: CalendarClock
...
export const AVAILABLE_CONNECTION_TYPES: ConnectionType[] = ['DSL', 'Cable', 'Fiber', 'Mobile'];
