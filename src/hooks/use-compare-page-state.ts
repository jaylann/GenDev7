...
export function useComparePageState(): ComparePageState {
    ...
    const providersForApi = useMemo(() => filters.selectedProviders.length > 0 ? filters.selectedProviders : [...AVAILABLE_PROVIDER_NAMES], [filters.selectedProviders],);
    ...
    const handleWebSocketSlugReceived = useCallback((slug: string | null, slugType: SlugType) => {
        if (!slug) return;

        setActiveShareableSlug(slug);
        setCurrentDisplaySlug(slug);

        const currentSearchLabel = sessionIdRef.current?.startsWith('shared-') ? null : sessionIdRef.current ? sessionIdRef.current : null;

        if (currentSearchLabel && !currentSearchLabel.startsWith('shared-')) {
            ...
            if (url) {
                if (slugType === 'INITIAL' && !hasAddedInitialHistoryEntryRef.current) {
                    ...
                    addRecentSearch({url, label: currentSearchLabel, sessionId: currentSearchLabel});
    ...
    const handleAddressSelected = useCallback((addr: ParsedAddress | null, fullText: string) => {
        ...
        const addressText = addr ? `${addr.street} ${addr.house_number}, ${addr.plz} ${addr.city}` : fullText.trim();
    ...
    const handleShowPendingOffers = useCallback(() => {
        if (pendingOffers) {
            ...
            if (activeShareableSlug && sessionIdRef.current) {
                ...
                if (url) addRecentSearch({
                    url, label: sessionIdRef.current, sessionId: sessionIdRef.current,
                });
            }
        }
        setPendingOffers(null);
        setIsUpdatePromptOpen(false);
        setIsRefiningOffers(false);
    }, [pendingOffers, activeShareableSlug, sortOption, filters, addRecentSearch]);
    ...
    const handleShareSingleOffer = useCallback(async (offer: Offer) => {
        ...
        try {
            const {shared_slug} = await generateShareLink(activeShareableSlug, offerKey);
            const url = buildUrl(shared_slug, 'recommended', DEFAULT_FILTERS, true);
            if (!url) {
    ...
    useEffect(() => {
        if (!activeShareableSlug || !sessionIdRef.current || sessionIdRef.current.startsWith('shared-') || isBlockingUi) return;
    ...
    }, [sortOption, filters, activeShareableSlug, isBlockingUi, addRecentSearch]);

    /* ───────────── derived helpers ───────────── */
    const isSearchButtonDisabled = isBlockingUi || !parsedBackendAddress || !GOOGLE_MAPS_API_KEY_FROM_ENV;

    const isSharePageDisabled = !activeShareableSlug || isBlockingUi || sharedLinkCopied || (originalOffers.length === 1 && currentDisplaySlug === activeShareableSlug);
