...
export const useRecentSearches = () => {
    ...
    useEffect(() => {
        ...
        const parseAndUpdateSearches = (stored: string | null): void => {
            if (!stored) {
                console.log("[useRecentSearches:useEffect] No stored searches found, initializing empty.");
                setRecentSearches([]);
                return;
            }
            try {
                ...
                if (
                    ...
                    const sortedSearches = parsed.sort((a, b) => b.timestamp - a.timestamp);
                    console.log("[useRecentSearches:useEffect] Successfully parsed and sorted searches from storage:", sortedSearches);
        ...
        console.log("[useRecentSearches:useEffect] Initializing recent searches from sessionStorage.");
        ...
        const handleStorageChange = (event: StorageEvent): void => {
            if (
                ...
                console.log("[useRecentSearches:useEffect] Storage event detected for key:", STORAGE_KEY);
        ...
        return () => {
            console.log("[useRecentSearches:useEffect] Cleaning up storage event listener.");
    ...
    const addRecentSearch = useCallback((data: AddRecentSearchData) => {
        ...
        setRecentSearches((prevSearches) => {
            console.log("[addRecentSearch] prevSearches:", JSON.parse(JSON.stringify(prevSearches.map(s => ({sessionId: s.sessionId, url: s.url, label: s.label})))));
            const canonicalUrl = getCanonicalCompareURL(data.url);
            console.log(`[addRecentSearch] Canonical URL for "${data.url}" is "${canonicalUrl}"`);
            ...
            if (existingIndex > -1) {
                ...
                console.log(`[addRecentSearch] Found existing item at index ${existingIndex} (sessionId: ${existingItem.sessionId}, url: ${existingItem.url}). Will update.`);
                ...
                updatedSearches = prevSearches.filter((s, i) => i !== existingIndex);
                updatedSearches.unshift(itemToUpdate);
                console.log(`[addRecentSearch] Updated item (sessionId: ${itemToUpdate.sessionId}, url: ${itemToUpdate.url}, label: "${itemToUpdate.label}"). Moved to top.`);
            } else {
                console.log(`[addRecentSearch] No existing item found for sessionId "${data.sessionId}" or url "${canonicalUrl}". Will add new.`);
                const newItem: RecentSearchItem = {
                    ...newEntryBase,
                    id: `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`, // Generate new ID
                };
                updatedSearches = [newItem, ...prevSearches];
                console.log(`[addRecentSearch] Added new item (sessionId: ${newItem.sessionId}, url: ${newItem.url}, label: "${newItem.label}").`);
            }
            ...
            if (updatedSearches.length > MAX_RECENT_SEARCHES) {
                ...
                console.log(`[addRecentSearch] List exceeded MAX_RECENT_SEARCHES. Trimmed. Removed items:`, removedItems.map(s => ({sessionId: s.sessionId, label: s.label})));
            }
            ...
            try {
                ...
                console.log("[addRecentSearch] Successfully persisted updated searches to sessionStorage.");
            } catch (err) {
                console.error(
    ...
    const updateSearchSlug = useCallback(
        (sessionId: string, newUrlFromCaller: string) => {
            if (!sessionId) {
                console.warn("[updateSearchSlug] ABORTED: No sessionId provided. newUrlFromCaller:", newUrlFromCaller);
                return;
            }

            setRecentSearches((prevSearches) => {
                console.groupCollapsed(`[updateSearchSlug] Attempting update for sessionId: "${sessionId}" with newUrlFromCaller: "${newUrlFromCaller}"`);
                console.log("[updateSearchSlug] Current prevSearches (before update):", JSON.parse(JSON.stringify(prevSearches.map(s => ({id:s.id, sessionId: s.sessionId, url: s.url, label:s.label, timestamp:s.timestamp})))));
                ...
                const newCanonicalUrl = getCanonicalCompareURL(newUrlFromCaller);
                const newSlug = extractSlug(newCanonicalUrl);

                console.log(`[updateSearchSlug] Item to update (sessionId "${sessionId}"):`, JSON.parse(JSON.stringify(originalItem)));
                console.log(`[updateSearchSlug] Original details: url="${originalItem.url}", slug="${originalSlug}"`);
                console.log(`[updateSearchSlug] New details from caller: newUrlFromCaller="${newUrlFromCaller}", newCanonicalUrl="${newCanonicalUrl}", newSlug="${newSlug}"`);


                // **CRUCIAL DUPLICATE SLUG CHECK**
                if (newSlug && newSlug !== originalSlug) {
                    const conflictingItem = prevSearches.find(
                        (s) => s.sessionId !== sessionId && extractSlug(s.url) === newSlug
                    );
                    if (conflictingItem) {
                        console.warn(
                    ...
                    console.log(`[updateSearchSlug] Slug for sessionId "${sessionId}" will change from "${originalSlug}" to "${newSlug}". No conflict found with other items.`);
                } else if (newSlug && newSlug === originalSlug) {
                    console.log(`[updateSearchSlug] The new slug "${newSlug}" is the same as the original slug for sessionId "${sessionId}". URL might change due to other params, or only timestamp update needed.`);
                } else if (!newSlug) {
                    console.log(`[updateSearchSlug] The new URL "${newCanonicalUrl}" does not contain a slug. Original slug was "${originalSlug}". This means the item might lose its slug.`);
                }
                ...
                if (originalItem.url === newCanonicalUrl) {
                    console.log(`[updateSearchSlug] Canonical URL for sessionId "${sessionId}" ("${newCanonicalUrl}") is identical to original. Will update timestamp and reorder.`);
                } else {
                    console.log(`[updateSearchSlug] Canonical URL for sessionId "${sessionId}" will change from "${originalItem.url}" to "${newCanonicalUrl}".`);
                }
                ...
                console.log(
                    ...
                    `Updated Item:`, JSON.parse(JSON.stringify(updatedItem))
                );
                ...
                if (newUpdatedSearches.length > MAX_RECENT_SEARCHES) {
                    console.warn("[updateSearchSlug] List somehow grew beyond MAX_RECENT_SEARCHES during an update. Trimming.");
                    newUpdatedSearches = newUpdatedSearches.slice(0, MAX_RECENT_SEARCHES);
                }
                ...
                try {
                    ...
                    console.log("[updateSearchSlug] Successfully persisted updated searches to sessionStorage.");
                } catch (err) {
                    console.error(
    ...
    const clearRecentSearches = useCallback(() => {
        console.log("[clearRecentSearches] Called. Clearing all recent searches.");
        setRecentSearches([]);
        try {
            sessionStorage.removeItem(STORAGE_KEY);
            console.log("[clearRecentSearches] Successfully removed recent searches from sessionStorage.");
        } catch (err) {
            console.error(
