...
import {AVAILABLE_CONNECTION_TYPES, AVAILABLE_CONTRACT_DURATIONS} from "@/config/constants";
...
export const serializeFiltersForURL = (filters: FiltersState): string => {
    ...
    if (filters.contractDurations.length > 0) params.set('cd', filters.contractDurations.join(','));
    if (filters.connectionTypes.length > 0) params.set('ct', filters.connectionTypes.join(','));
    if (filters.minSpeed > 0) params.set('ms', String(filters.minSpeed));
    if (filters.tvIncluded !== 'any') params.set('tv', filters.tvIncluded);
    if (filters.selectedProviders.length > 0) params.set('sp', filters.selectedProviders.map(encodeURIComponent).join(','));
 ...
 */
export const deserializeFiltersFromURL = (searchParams: URLSearchParams): Partial<FiltersState> => {
    const loadedFilters: Partial<FiltersState> = {};
    if (searchParams.has('cd')) loadedFilters.contractDurations = searchParams.get('cd')!.split(',').map(Number).filter(n => !isNaN(n) && AVAILABLE_CONTRACT_DURATIONS.includes(n));
    if (searchParams.has('ct')) loadedFilters.connectionTypes = searchParams.get('ct')!.split(',').filter(ct => AVAILABLE_CONNECTION_TYPES.includes(ct as ConnectionType)) as ConnectionType[];
    ...
    if (searchParams.has('tv')) {
        ...
        if (['any', 'yes', 'no'].includes(tvVal)) loadedFilters.tvIncluded = tvVal;
    }
    if (searchParams.has('sp')) loadedFilters.selectedProviders = searchParams.get('sp')!.split(',').map(decodeURIComponent);
