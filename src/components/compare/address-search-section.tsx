...
import {Search as SearchIcon, Loader, LoaderCircle} from 'lucide-react';
import {
...
export const AddressSearchSection: FC<AddressSearchSectionProps> = ({
    ...
    const handleInternalAddressSelect = (
        address: AutocompleteParsedAddress | null,
        fullText: string
    ): void => {
        ...
        <section className="max-w-2xl mx-auto">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                <AddressAutocompleteInput
                    ...
                    disabled={!hasApiKey || isLoadingFromSlug || isLoading} // Also disable if normal search is loading
                />
                <Button
                    ...
                    size="lg"
                >
                    {isLoading && !currentSlug && !isLoadingFromSlug ? ( // Normal search loading (isWaitingInitialOffers)
                        <><LoaderCircle className="animate-spin w-5 h-5 mr-2" />Searching...</>
                    ) : isLoadingFromSlug ? ( // Loading shared link (isLoadingFromUrl)
                        <><LoaderCircle className="animate-spin w-5 h-5 mr-2" />Loading...</>
                    ) : (
