...
// -----------------------------------------------------------------------------
export const GoogleMapsLoader: React.FC = () => apiKey ? (<Script
        id="google-maps-sdk"
...
//  Helpers (unchanged)
// -----------------------------------------------------------------------------
const extractAddressComponent = (comps: google.maps.GeocoderAddressComponent[], type: string, short = false,): string | undefined => comps.find((c) => c.types.includes(type))?.[short ? 'short_name' : 'long_name'];
...
const parseGeocodeResult = (results: google.maps.GeocoderResult[]): Address | null => {
    ...
    const city = extractAddressComponent(c, 'locality') ?? extractAddressComponent(c, 'postal_town') ?? extractAddressComponent(c, 'administrative_area_level_3') ?? extractAddressComponent(c, 'sublocality_level_1');
    const plz = extractAddressComponent(c, 'postal_code');

    if (!street || !house || !city || !plz || !/^\d{5}$/.test(plz)) {
        console.warn('[ParseGeocode] Validation failed for a DE address:', {
            street,
            house,
            city,
            plz,
...
export const AddressAutocompleteInput: React.FC<AddressAutocompleteInputProps> = ({
    ...
    const {
        ready, value, // This value from the hook should always be a string
        suggestions: {status, data, loading: suggestionsLoading}, setValue, clearSuggestions, init,
    ...
    return (<div className={cn('relative w-full', containerClassName)}>
            ...
            {showSuggestions && status === 'OK' && data.length > 0 && (<ul ref={listRef}
                    ...
                    {data.map(({place_id, description, structured_formatting}) => (
                        <li key={place_id} onMouseDown={(e) => e.preventDefault()}
                            onClick={() => handleSelect(description)}
                            className="cursor-pointer px-4 py-2.5 text-sm text-slate-200 hover:bg-indigo-600 hover:text-white"
                            role="option" aria-selected={false}>
                            <strong>{structured_formatting.main_text}</strong> <small
                            className="text-slate-400">{structured_formatting.secondary_text}</small>
                        </li>))}
                </ul>)}
            ...
            {showSuggestions && value.trim() && !suggestionsLoading && (<>
                    {status === 'ZERO_RESULTS' && (<div
                            className="absolute z-20 mt-1 w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-400 shadow-lg">
                            No matching addresses found.
                        </div>)}
                    {status === 'REQUEST_DENIED' && (<div
                            className="absolute z-20 mt-1 w-full rounded-md border border-red-700 bg-red-900/50 px-3 py-2 text-sm text-red-400 shadow-lg">
                    ...
                    {status && status !== 'OK' && status !== 'ZERO_RESULTS' && status !== 'REQUEST_DENIED' && (<div
                            className="absolute z-20 mt-1 w-full rounded-md border border-orange-600 bg-orange-900/50 px-3 py-2 text-sm text-orange-300 shadow-lg">
