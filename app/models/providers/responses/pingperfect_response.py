...
class PingPerfectResponse(BaseModel):
    ...
    price_cents_month: OptPosInt = Field(None, description="Introductory monthly price in cents")
    contract_duration_months: PosInt = Field(..., description="Minimum contract term in months")
    installation_service_included: bool = Field(..., description="Whether installation service is included")
