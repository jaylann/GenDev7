...
class ByteMeResponse(BaseModel):
    ...
    price_cents_month_intro: OptPosInt = Field(
        default=None, description="Introductory monthly price in cents"
    )
    price_cents_month_regular: OptPosInt = Field(
        default=None, description="Regular monthly price in cents"
    )
    ...
    installation_service_included: bool = Field(
        default=False, description="Whether installation service is included"
    )
    tv_included: bool = Field(default=False, description="Whether TV is included")
    tv_package_name: OptStrClean = Field(
        default=None, description="Name of the TV package if included"
    )
    data_cap_gb: OptPosInt = Field(default=None, description="Data cap in GB")
    voucher_type: OptStrClean | VoucherKind = Field(
        default=None, description="Type of voucher/incentive"
    )
    voucher_value_cents: OptPosInt = Field(
        default=None, description="Absolute voucher value in cents or cashback amount"
    )
    voucher_value_percent: OptPercent = Field(
        default=None, description="Percentage voucher value (0–100%)"
    )
    max_age: OptPosInt = Field(
        default=None, description="Maximum customer age to qualify for voucher"
    )
