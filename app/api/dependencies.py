...
async def get_providers(provider_names: Optional[List[str]] = None, wants_fiber: bool = False) -> List[ProviderBase]:
    ...
    if provider_names:
        providers: List[ProviderBase] = [p for p in all_providers if p.name in provider_names]
    else:
        providers: List[ProviderBase] = all_providers

    logger.info(
        f"Loaded providers: {[p.name for p in providers]}; "
    )
    return providers
