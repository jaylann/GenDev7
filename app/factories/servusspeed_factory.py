    ...
    def parse_detail_response(pid: str, payload: Any) -> Optional[ServusSpeedResponse]:
        ...
        try:
            if not isinstance(payload, dict):
                ...
                logger.error(f"Invalid payload type for pid {pid}: expected dict, got {type(payload).__name__}")
                return None

            prod = payload.get("servusSpeedProduct")
            if not isinstance(prod, dict):
                logger.error(f"Invalid payload for pid {pid}: missing or invalid 'servusSpeedProduct'")
                return None

            info = prod.get("productInfo")
            if not isinstance(info, dict):
                logger.error(f"Invalid payload for pid {pid}: missing or invalid 'productInfo'")
                return None

            price = prod.get("pricingDetails")
            if not isinstance(price, dict):
                logger.error(f"Invalid payload for pid {pid}: missing or invalid 'pricingDetails'")
                return None
            ...
            def to_int(value, name):
                try:
                    ...
                    logger.warning(f"Invalid integer '{value}' for field '{name}' in pid {pid}")
            ...
            if not isinstance(connection_type, str):
                connection_type_str = str(connection_type).replace("{", "{{").replace("}", "}}")
                logger.error(f"Invalid connectionType '{connection_type_str}' for pid {pid}")
                return None
            ...
            tv_package_name = tv_package_name_candidate if tv_package_name_candidate else None
            ...
            if isinstance(installation_val, bool):
                ...
                installation_service_included = str(installation_val).strip().lower() in ("yes", "true", "included")
            ...
            logger.warning(f"Type error parsing Servus Speed response for pid {pid}: {e}")
            return None
        except Exception as e: # Catch any other unexpected error
            logger.warning(f"Unexpected error parsing Servus Speed response for pid {pid}: {e}")
            return None